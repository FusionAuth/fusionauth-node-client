...
RESTClient.prototype = {
  ...
  basicAuthorization: function(username, password) {
    if (username && password) {
      this.header('Authorization', 'Basic ' + Buffer.from(username + ':' + password).toString('base64'));
    }
    return this;
  },
  ...
  /**
   * Sets the form data body of the client request.
   *
   * @param {Object} body The object to be written to the request body as form data (x-www-url-formencded).
   * @returns {RESTClient}
   */
  ...
  /**
   * @callback RESTClient~callback
   *
   * @template {T}
   *
   * @param {ClientResponse<T>} clientResponse The response when the request has completed or failed.
   */
   ...
   * @template {T}
   *
   * @param {RESTClient~callback<T>} responseHandler The response handler function callback.
   */
    ...
    const request = myHttp.request(options, function(response) {
      ...
      response.on('data', function(data) {
        ...
        if (clientResponse.wasSuccessful()) {
          clientResponse.successResponse = json || null;
        } else {
          clientResponse.errorResponse = json || null;
        }
